...
import {
    ...
    FormLabel, Select, Input, useToast, Stack, Skeleton
                ...
                <Flex justify="center">
                    ...
                    <Box width="100%">
                        ...
                        {isLoading ? loadingVisual : data.filter(t => t.frequency === 'Daily').map((task, index) => {
                            ...
                            return (
                                <CategoryTask key={index} category={task.category} taskNames={names}
                                              completeTasks={selected} tags={[]} frequency="Daily" type="master"
                                              showAlert={showAlert}/>
                            )
                        })}
                    </Box>
                        ...
                        {isLoading ? loadingVisual : data.filter(t => t.frequency === 'Weekly').map((task, index) => {
                            ...
                            return (
                                <CategoryTask key={index} category={task.category} tasks={task.tasks} taskNames={names}
                                              completeTasks={selected} tags={[]} frequency="Weekly" type="master"
                                              showAlert={showAlert}/>
                            )
                        })}
                    </Box>
